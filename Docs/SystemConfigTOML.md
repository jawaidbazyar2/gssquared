...
| `"serial"` | Real host UART; `path` is the OS port name (macOS `/dev/cu.*`, Windows `COMn`, Linux `/dev/ttyS*` `/dev/ttyUSB*` `/dev/ttyACM*` `/dev/ttyAMA*`) |
...
Supported image types are whatever `identify_media()` accepts (`.po`, `.dsk`, `.woz`, `.2mg`, `.hdv`, `.hda`, `.iso`, `.pmap` etc.). APM `.iso` / block images expand to multiple BazFast units the same way a `.pmap` does.
