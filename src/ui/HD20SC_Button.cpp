 ...
 */
void HD20SC_Button_t::render() {
    this->set_assetID(Apple_HD20SC);
    ...
    if (status.is_mounted && status.motor_on) {
        ...
        snprintf(text, sizeof(text), "%d/%d %06X", key.slot, key.drive+1, status.position / 2);
    } else {
        snprintf(text, sizeof(text), "%d/%d", key.slot, key.drive+1);
    }
    float text_width = (strlen(text) * 8.0);
    ctx->debug_text(text, tp.x + cp.x + (tp.w - text_width)/2, tp.y + cp.y + 38, 0x000000FF);
}
