...
protected:
    // Share IIe delay table (same Mega II softswitch latency class).
    static constexpr uint8_t delay_lut_iiepal[static_cast<uint8_t>(vs_mode_switch_t::COUNT)] = {
    ...
    inline uint8_t mode_change_delay(vs_mode_switch_t sw) const override {
        return delay_lut_iiepal[static_cast<uint8_t>(sw)];
    }
