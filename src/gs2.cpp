...
SDL_AppResult SDL_AppInit(void **appstate, int argc, char **argv) {
    ...
    // If the caller passed `-c file.gs2` or `*Settings.txt`, skip the
    // system-selector UI and jump straight into emulation using the loaded
    // configuration.
    if (state->loaded_config) {
        std::cout << "Auto-launching system config: " << config_path << std::endl;
        transition_to_emulation(state, &state->loaded_config->config(), -1);
        state->auto_launched = true;
    } else if (platform_explicit || gs2_app_values.headless) {
        ...
        // whose platform_id matches. Headless mode always auto-launches
        // (there is no selector UI to interact with), defaulting to the
        // platform_id above.
